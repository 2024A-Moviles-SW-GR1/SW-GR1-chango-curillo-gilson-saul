    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        guardarButton.setOnClickListener {
            ...
            val values = ContentValues().apply {
                put(DbHelper.COLUMN_MARCA, newMarca)
                put(DbHelper.COLUMN_MODELO, newModelo)
                put(DbHelper.COLUMN_ANIO_LANZAMIENTO, newAnioLanzamiento)
                put(DbHelper.COLUMN_PRECIO, newPrecio)
                put(DbHelper.COLUMN_ES_5G, if (newEs5G) 1 else 0)
            }
            val selection = "${DbHelper.COLUMN_MARCA} = ? AND ${DbHelper.COLUMN_MODELO} = ?"
            val selectionArgs = arrayOf(originalMarca, originalModelo)
            val rowsUpdated = db.update(DbHelper.TABLE_CELULAR, values, selection, selectionArgs)
            db.close()
